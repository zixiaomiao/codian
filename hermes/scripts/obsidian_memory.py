...
# 统一分类注册表 —— 所有分类相关的函数从此 dict 派生
# 加一个分类只需在这里加一条，classify_log / categories_for_query / category_purpose 自动同步
CATEGORIES = {
    "Project": {
        "path": MEMORY_ROOT_REL / "20-Memory-记忆/project.md",
        "purpose": "项目概况、发布记录、安装流程、当前状态和近期变更。",
        "classify_match": ["project", "项目", "release", "readme", "插件", "仓库", "安装", "发布", "summary"],
        "query_match": ["project", "项目", "summary", "release", "readme", "发布", "安装"],
    },
    "Decision": {
        "path": MEMORY_ROOT_REL / "20-Memory-记忆/decisions.md",
        "purpose": "长期有效的技术决策、同步策略、读取策略和范围边界。",
        "classify_match": ["decision", "决定", "选择", "策略", "规则", "默认"],
        "query_match": ["decision", "决策", "为什么", "策略", "规则", "选择"],
    },
    "Todo": {
        "path": MEMORY_ROOT_REL / "20-Memory-记忆/todos.md",
        "purpose": "后续规划、待办事项和路线图。",
        "classify_match": ["todo", "待办", "roadmap", "规划", "下一阶段", "后续推进"],
        "query_match": ["todo", "待办", "roadmap", "规划", "下一步"],
    },
    "Bug": {
        "path": MEMORY_ROOT_REL / "20-Memory-记忆/bugs-and-fixes.md",
        "purpose": "错误、失败原因、修复记录、兼容性问题和验证结果。",
        "classify_match": ["bug", "fix", "修复", "错误", "失败", "报错", "兼容"],
        "query_match": ["bug", "修复", "错误", "失败", "报错", "兼容"],
    },
    "User Preference": {
        "path": MEMORY_ROOT_REL / "20-Memory-记忆/user-preferences.md",
        "purpose": "用户偏好、固定路径、操作习惯、安全规则和回答风格。",
        "classify_match": ["preference", "偏好", "用户偏好", "先完成任务", "少废话"],
        "query_match": ["preference", "偏好", "路径", "安全", "少废话", "用户"],
    },
}
# MEMORY_CATEGORY_RELS 保持兼容，从 CATEGORIES 自动派生
MEMORY_CATEGORY_RELS = {name: info["path"] for name, info in CATEGORIES.items()}
...
def prompt_for_vault() -> Path:
    ...
    print("Where is your Obsidian vault? (Codian Memory will be created there)")
    raw = input("Vault path> ").strip()
    if not raw:
        raise SystemExit("No path entered.")
    chosen = canonical_vault_root(Path(raw).expanduser())
...
def classify_log(block: str) -> List[str]:
    ...
    for name, info in CATEGORIES.items():
        if any(term in payload for term in info["classify_match"]):
...
def generate_memory_categories(max_logs: int = 80) -> List[Path]:
    ...
    # 交叉检查：buckets 中有没有 category_paths 不认识的分类
    cat_paths = category_paths()
    orphans = set(buckets) - set(cat_paths)
    if orphans:
        orphan_count = sum(len(buckets[k]) for k in orphans)
        print(
            f"⚠️  警告：以下分类在 CATEGORIES 注册表中无对应文件路径，"
            f"{orphan_count} 条数据不会被写入：{', '.join(sorted(orphans))}",
            file=sys.stderr,
        )

    for name, path in cat_paths.items():
        path.parent.mkdir(parents=True, exist_ok=True)
...
def category_purpose(name: str) -> str:
    ...
    return info["purpose"] if info else "分类记忆。"

def categories_for_query(query: str) -> List[str]:
    lower = query.lower()
    matched = []
    for name, info in CATEGORIES.items():
        if any(term in lower for term in info["query_match"]):
            matched.append(name)
    return matched
